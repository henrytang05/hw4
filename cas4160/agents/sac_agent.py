...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            assert self.num_critic_networks == 2, (
                "Double-Q backup only works with 2 critic networks"
            )
            # Swap the two critics' predictions so that each critic uses the other as the target.
    ...
    def update_critic(
        ...
        with torch.no_grad():
            # DONE: (student): Sample from the actor
    ...
    def entropy(
        self, action_distribution: torch.distributions.Distribution
    ) -> torch.Tensor:
        """
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            # DONE: \(student\): Draw self.num_actor_samples samples from the action distribution for each batch element
            # NOTE: think about whether to use .rsample() or .sample() here
            ...
            q_values = self.critic(
                obs[None].expand((self.num_actor_samples, batch_size, *obs.shape[1:])),
                action,
            )
        ...
        # DONE: \(student\): Calculate log-probs
        log_probs = action_distribution.log_prob(action)
        assert log_probs.shape == q_values.shape

        # DONE: \(student\): Compute policy gradient using log-probs and Q-values
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # DONE: \(student\): Compute the actor loss using Q-values
        loss = -1.0 * q_values.min(dim=0).values.mean()
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        if self.actor_gradient_type == "reparametrize":
            ...
            loss, entropy = 0.0, self.entropy(self.actor(obs)).mean()
